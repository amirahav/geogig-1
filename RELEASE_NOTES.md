...
the canonical tree contents, but using a quad-tree clustering strategy for it's non terminal
nodes.
...
* Spatial indexing command set: the geogig command line and it's WEB API include a full set of
...
* Better support for foreign CRS definitions. Importing from datasets that define it's coordinate
